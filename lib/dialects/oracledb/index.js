...
class Client_Oracledb extends Client_Oracle {
  ...
  acquireRawConnection(connectionSettings = this.connectionSettings) {
    return new Promise((resolver, rejecter) => {
      ...
      oracleDbConfig.connectString = resolveConnectString(connectionSettings);

      if (connectionSettings.prefetchRowCount) {
        oracleDbConfig.prefetchRows = connectionSettings.prefetchRowCount;
      }

      if (connectionSettings.stmtCacheSize !== undefined) {
  ...
  async acquireConnection(context) {
    const connection = await super.acquireConnection(context);
