...
const parseConnection = require('./knex-builder/internal/parse-connection');
...
class Client extends EventEmitter {
  constructor(config = {}) {
    ...
    this._connectionEntries = undefined;
    this._defaultConnectionName = undefined;
    ...
    const hasConnections =
      config.connections && Object.keys(config.connections).length > 0;

    if (hasConnections && config.connection) {
      throw new Error(
    ...
    if (hasConnections) {
      this._initializeConnectionsConfig(config);
    ...
    if (this.driverName && (config.connection || hasConnections)) {
  ...
  _initializeConnectionsConfig(config) {
    if (this.connectionResolver && typeof this.connectionResolver !== 'function') {
      throw new Error(
        `knex: connectionResolver must be a function when provided`
      );
    }

    const connectionsConfig = config.connections || {};
    const connectionNames = Object.keys(connectionsConfig);
    if (connectionNames.length === 0) {
      throw new Error(
        'knex: connections configuration must include at least one connection'
      );
    }

    this._connectionEntries = Object.create(null);
    for (const name of connectionNames) {
      const entry = connectionsConfig[name];
      const hasWrapper =
        entry &&
        typeof entry === 'object' &&
        !Array.isArray(entry) &&
        ('connection' in entry || 'pool' in entry);
      const connection = hasWrapper ? entry.connection : entry;
      const pool = hasWrapper ? entry.pool : undefined;

      if (!connection) {
        throw new Error(
          `knex: connection "${name}" is missing a connection configuration`
        );
      }

      this._connectionEntries[name] = {
        connection: this._normalizeConnectionSettings(connection),
        pool,
      };
    }

    this._defaultConnectionName = this._resolveDefaultConnectionName(
      connectionsConfig
    );
    ...
    if (
      this._defaultConnectionName &&
      this._connectionEntries[this._defaultConnectionName]
    ) {
      const defaultConnection =
        this._connectionEntries[this._defaultConnectionName].connection;
      if (
        defaultConnection &&
        typeof defaultConnection === 'object' &&
        !Array.isArray(defaultConnection) &&
        typeof defaultConnection !== 'function'
      ) {
        this.connectionSettings = defaultConnection;
      }
    }
  ...
  _resolveDefaultConnectionName(connectionsConfig) {
    if (!connectionsConfig) {
      return undefined;
    }

    if (connectionsConfig.default) {
      return 'default';
    }

    if (connectionsConfig.master) {
      return 'master';
    }

    const names = Object.keys(connectionsConfig);
    if (names.length === 1) {
      return names[0];
    }
  ...
  _normalizeConnectionSettings(connectionSettings) {
    if (typeof connectionSettings === 'string') {
      connectionSettings = parseConnection(connectionSettings).connection;
    }
    if (
      connectionSettings &&
      typeof connectionSettings === 'object' &&
      !Array.isArray(connectionSettings) &&
      typeof connectionSettings !== 'function'
    ) {
      const cloned = cloneDeep(connectionSettings);
      if (connectionSettings.password) {
        setHiddenProperty(cloned, connectionSettings);
      }
      if (
        connectionSettings.authentication &&
        connectionSettings.authentication.options &&
        connectionSettings.authentication.options.password
      ) {
        setHiddenProperty(
          cloned.authentication.options,
          connectionSettings.authentication.options
        );
      }
      if (cloned.expirationChecker) {
        delete cloned.expirationChecker;
      }
      return cloned;
    }
  ...
  async _resolveConnectionSettings(connectionName) {
    if (!this._connectionEntries) {
      return this.connectionSettings;
    }

    const entry = this._connectionEntries[connectionName];
    if (!entry) {
      return undefined;
    }
    ...
    if (typeof connection === 'function') {
      ...
      return this._normalizeConnectionSettings(result);
    }

    return connection;
  }

  async _resolveConnectionName(context) {
    if (!this._connectionEntries) {
      return undefined;
    }

    if (context && context.connectionName) {
      return context.connectionName;
    }

    if (this.connectionResolver) {
      const resolved = await this.connectionResolver(context || {});
  ...
  _getPoolForConnection(connection) {
    if (this.pools) {
      const connectionName =
        connection && connection.__knexConnectionName
          ? connection.__knexConnectionName
          : this._defaultConnectionName;
      if (connectionName && this.pools[connectionName]) {
  ...
  getPoolSettings(poolConfig, poolContext = {}) {
    const { connectionName, resolveConnectionSettings } = poolContext;
    const {
    ...
    const updatePoolConnectionSettingsFromProvider = async (
      ...
      if (resolveConnectionSettings) {
        return resolveConnectionSettings(forceRefresh);
      }
      if (!this.connectionConfigProvider) {
        return this.connectionSettings; // static configuration, nothing to update
      }
      if (
        !forceRefresh &&
        ...
        return this.connectionSettings; // not expired, reuse existing connection
      }
    ...
    const tarnPoolConfig = {
      ...
      create: async () => {
        const connectionSettings = await updatePoolConnectionSettingsFromProvider();
        const connection = await this.acquireRawConnection(connectionSettings);
        connection.__knexUid = uniqueId('__knexUid');
        if (connectionName) {
          connection.__knexConnectionName = connectionName;
          if (connectionSettings !== undefined) {
            connection.__knexConnectionSettings = connectionSettings;
          }
        }
        if (hasMaxConnectionLifetime) {
          connection.__knexLifetimeLimit = generateLifetimeLimit();
  ...
  initializePools(config = this.config) {
    if (this.pools) {
      this.logger.warn('The pools have already been initialized');
      return;
    }

    const connections = this._connectionEntries || {};
    this.pools = Object.create(null);

    for (const [name, entry] of Object.entries(connections)) {
      const poolConfig = defaults({}, entry.pool, config.pool);
      this.pools[name] = new Pool(
        this.getPoolSettings(poolConfig, {
          connectionName: name,
          resolveConnectionSettings: () => this._resolveConnectionSettings(name),
        })
      );
    }

    if (this._defaultConnectionName && this.pools[this._defaultConnectionName]) {
      this.pool = this.pools[this._defaultConnectionName];
    }
  }
  ...
  async acquireConnection(context = {}) {
    const hasMultiplePools = Boolean(this.pools);
    let pool = this.pool;
    let connectionName;
    ...
    if (hasMultiplePools) {
      ...
      connectionName = await this._resolveConnectionName(context);
      if (!connectionName) {
        throw new Error(
          'Unable to acquire a connection: connectionResolver did not return a connection name and no default connection is configured'
        );
      }
      pool = this.pools[connectionName];
      if (!pool) {
        throw new Error(
          `Unable to acquire a connection: unknown connection "${connectionName}"`
        );
      }
    }
    ...
    try {
      ...
      if (connectionName && !connection.__knexConnectionName) {
        connection.__knexConnectionName = connectionName;
      }
  ...
  releaseConnection(connection) {
    ...
    const pool = this._getPoolForConnection(connection);
    if (!pool) {
      ...
      if (this.pools) {
        await Promise.all(
          Object.values(this.pools).map((pool) =>
            pool && pool.destroy ? pool.destroy() : undefined
          )
        );
        this.pools = undefined;
      } else if (this.pool && this.pool.destroy) {
  ...
  database() {
    if (this.connectionSettings && this.connectionSettings.database) {
      return this.connectionSettings.database;
    }

    if (
      this._connectionEntries &&
      this._defaultConnectionName &&
      this._connectionEntries[this._defaultConnectionName]
    ) {
      const defaultConnection =
      ...
      if (
        ...
        return defaultConnection.database;
      }
    }
